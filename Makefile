...
NVCC_FLAGS := -G ${CC_FLAGS} -arch=sm_21  -ccbin ${CXX}
...
sigcache: sigcache_host_nvcc sigcache_device 
	@for prog in $^; do \
		echo "---------\nRunning $$prog\n---------"; \
		./$$prog; \
	done
...
sigcache_device: sigcache_test.cpp sigcache.h
	@${NVCC} ${LIB_FLAGS} -x cu ${CC_FLAGS} ${NVCC_FLAGS} $< -o $@
...
sigcache_host_nvcc: sigcache_test.cpp sigcache.h
	@${NVCC} ${LIB_FLAGS} -x c++ ${CC_FLAGS} ${NVCC_FLAGS} $< -o $@
